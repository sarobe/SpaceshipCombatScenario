...
import controller.mcts.IRoller;
import controller.singleMCTS.SingleMCTSPlayer;
...
public class SingleTreeNode
{
    ...
    public SingleTreeNode(IGameState state, IRoller roller, TreePolicy treePolicy, Random rnd,
                          Player a_player, PlayoutInfo pi) {
        this(state, null, -1, roller, treePolicy, rnd, a_player,pi);
    }

    public SingleTreeNode(IGameState state, SingleTreeNode parent, int childIndex, IRoller roller,
    ...
    public SingleTreeNode treePolicy() {
        ...
        try{
            while (cur.nonTerminal() && !cur.state.isTerminal() && depth < SingleMCTSParameters.ROLLOUT_DEPTH)
            {
    ...
    public SingleTreeNode expand() {
        ...
        while(tn == null && prunedN < m_prunedChildren.length)
        {
            ...
            IGameState nextState = state.copy();
    ...
    {
        IGameState rollerState = state.copy();
    ...
    public void advance(IGameState st, int action)
    {
    ...
    public boolean finishRollout(IGameState rollerState, int depth, int action)
    {
